...
# Wickandbergamot talk to each other?
#
...
for v in "${otherVersions[@]}"; do
  wickandbergamot-install-init "${solanaInstallGlobalOpts[@]}" "$v"
  wickandbergamot -V
done
...
solanaInstallUse() {
  declare version=$1
  echo "--- Now using WICKANDBERGAMOT $version"
  WICKANDBERGAMOT_BIN="$solanaInstallDataDir/releases/$version/solana-release/bin"
  export PATH="$WICKANDBERGAMOT_BIN:$ORIGINAL_PATH"
}
...
(
  ...
  if [[ ! -x baseline-run.sh ]]; then
    curl https://raw.githubusercontent.com/wickandbergamot/wickandbergamot/v"$baselineVersion"/run.sh -o baseline-run.sh
  ...
  wickandbergamot --url http://127.0.0.1:8328 show-validators
)
...
for v in "${otherVersions[@]}"; do
  ...
  (
    set -x
    "$WICKANDBERGAMOT_BIN"/wickandbergamot-gossip spy \
...
#
# Once https://github.com/wickandbergamot/wickandbergamot/issues/7738 is resolved, remove
...
for v in "${otherVersions[@]}"; do
  ...
  (
    ...
    tmux new-window -t abi -n "$v" " \
      $WICKANDBERGAMOT_BIN/wickandbergamot-validator \
      --ledger $ledger \
      --no-snapshot-fetch \
      --entrypoint 127.0.0.1:10015 \
