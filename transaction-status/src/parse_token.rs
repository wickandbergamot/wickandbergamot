use {
    ...
    wickandbergamot_account_decoder::parse_token::{
        pubkey_from_wickandbergamot_token, token_amount_to_ui_amount, UiAccountState,
    },
    solana_sdk::{
    ...
    wickandbergamot_token_2022::{
        ...
        solana_program::{
            instruction::Instruction as wickandbergamotTokenInstruction, program_option::COption,
            pubkey::Pubkey,
        },
    },
};
    ...
    match instruction.accounts.iter().max() {
        ...
        _ => {
            ...
            return Err(ParseInstructionError::InstructionKeyMismatch(
                ParsableProgram::WickandbergamotToken,
            ));
        }
    }
...
fn check_num_token_accounts(accounts: &[u8], num: usize) -> Result<(), ParseInstructionError> {
    check_num_accounts(accounts, num, ParsableProgram::WickandbergamotToken)
}

pub fn wickandbergamot_token_instruction(instruction: WickandbergamotTokenInstruction) -> Instruction {
    Instruction {
        program_id: pubkey_from_wickandbergamot_token(&instruction.program_id),
        accounts: instruction
            .accounts
            .iter()
            .map(|meta| AccountMeta {
                pubkey: pubkey_from_wickandbergamot_token(&meta.pubkey),
...
mod test {
    use {
        ...
        wickandbergamot_token_2022::{
            ...
            solana_program::{
                ...
                pubkey::Pubkey as WickandbergamotTokenPubkey,
            },
        },
        std::str::FromStr,
    };

    pub(super) fn convert_pubkey(pubkey: Pubkey) -> WickandbergamotTokenPubkey {
        WickandbergamotTokenPubkey::from_str(&pubkey.to_string()).unwrap()
    }

    pub(super) fn convert_compiled_instruction(
        instruction: &WickandbergamotTokenCompiledInstruction,
    ) -> CompiledInstruction {
        CompiledInstruction {
    ...
    fn test_parse_token(program_id: &WickandbergamotTokenPubkey) {
        ...
        let get_account_data_size_ix = get_account_data_size(
            program_id,
            &convert_pubkey(mint_pubkey),
            &[], // This emulates the packed data of wickandbergamot_token::instruction::get_account_data_size
        )
        .unwrap();
    ...
    fn test_parse_token_v3() {
        test_parse_token(&wickandbergamot_token::id());
    }
    ...
    fn test_token_ix_not_enough_keys(program_id: &WickandbergamotTokenPubkey) {
    ...
    fn test_not_enough_keys_token_v3() {
        test_token_ix_not_enough_keys(&wickandbergamot_token::id());
    }
