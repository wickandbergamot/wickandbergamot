# Distribute Wickandbergamot tokens
...
```bash
wickandbergamot-tokens distribute-tokens --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
wickandbergamot-tokens transaction-log --output-path transactions.csv
```
...
```bash
wickandbergamot-tokens distribute-tokens --dry-run --input-csv <RECIPIENTS_CSV>
```
...
```bash
wickandbergamot-tokens distribute-tokens --transfer-amount 10 --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
wickandbergamot-tokens distribute-stake --stake-account-address <ACCOUNT_ADDRESS> \
...
```bash
wickandbergamot-tokens safe-token-balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
...
```bash
wickandbergamot-tokens distribute-safe-tokens --from <ADDRESS> --owner <KEYPAIR> \
...
the `--dry-run` parameter, or `wickandbergamot-tokens balances`.
...
```bash
wickandbergamot-tokens balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
