...
use {
    ...
    wickandbergamot_account_decoder::parse_token::UiTokenAccount,
    wickandbergamot_clap_utils::keypair::SignOnly,
    wickandbergamot_client::rpc_response::{
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        for account in &self.accounts {
            writeln!(
                ...
                "{:<44}  {}",
                account.address,
                &format!("{} WICKANDBERGAMOT", lamports_to_sol(account.lamports))
            )?;
        }
        Ok(())
    }
...
impl fmt::Display for CliSupply {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        writeln_name_value(f, "Total:", &format!("{} WICKANDBERGAMOT", lamports_to_sol(self.total)))?;
        writeln_name_value(
            f,
            "Circulating:",
            &format!("{} WICKANDBERGAMOT", lamports_to_sol(self.circulating)),
        )?;
        writeln_name_value(
            f,
            "Non-Circulating:",
            &format!("{} WICKANDBERGAMOT", lamports_to_sol(self.non_circulating)),
        )?;
