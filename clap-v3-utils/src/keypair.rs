...
//! standard signing sources, from the command line, as in the Wickandbergamot CLI.
//!
...
//! sources supported by the Wickandbergamot CLI. Many other functions here are
...
impl DefaultSigner {
    ...
    /// use wickandbergamot_clap_v3_utils::keypair::DefaultSigner;
    /// use wickandbergamot_clap_v3_utils::offline::OfflineArgs;
    ///
            ...
            parse_signer_source(&self.path)
                ...
                .map_err(|_| {
                    std::io::Error::new(
                        ...
                        format!(
                        "No default signer found, run \"wickandbergamot-keygen new -o {}\" to create a new one",
                        self.path
                    ),
                    )
                })?;
    ...
    /// ```no_run
    /// use clap::{Arg, Command};
    /// use wickandbergamot_clap_v3_utils::keypair::{DefaultSigner, signer_from_path};
    /// use wickandbergamot_clap_v3_utils::offline::OfflineArgs;
    ...
    /// ```no_run
    /// use clap::{Arg, Command};
    /// use wickandbergamot_clap_v3_utils::keypair::{SignerFromPathConfig, DefaultSigner};
    /// use wickandbergamot_clap_v3_utils::offline::OfflineArgs;
    ///
    /// let clap_app = Command::new("my-program")
    ///     // The argument we'll parse as a signer "path"
...
/// ```no_run
/// use clap::{Arg, Command};
/// use wickandbergamot_clap_v3_utils::keypair::signer_from_path;
/// use wickandbergamot_clap_v3_utils::offline::OfflineArgs;
///
/// let clap_app = Command::new("my-program")
///     // The argument we'll parse as a signer "path"
...
/// ```no_run
/// use clap::{Arg, Command};
/// use wickandbergamot_clap_v3_utils::keypair::{signer_from_path_with_config, SignerFromPathConfig};
/// use wickandbergamot_clap_v3_utils::offline::OfflineArgs;
///
/// let clap_app = Command::new("my-program")
///     // The argument we'll parse as a signer "path"
    ...
    match kind {
        ...
        SignerSourceKind::Filepath(path) => match read_keypair_file(&path) {
            Err(e) => Err(std::io::Error::new(
                std::io::ErrorKind::Other,
                format!("could not read keypair file \"{}\". Run \"wickandbergamot-keygen new\" to create a keypair file: {}", path, e),
            )
            .into()),
...
/// ```no_run
/// use clap::{Arg, Command};
/// use wickandbergamot_clap_v3_utils::keypair::pubkey_from_path;
///
/// let clap_app = Command::new("my-program")
///     // The argument we'll parse as a signer "path"
///     .arg(Arg::new("keypair")
        ...
        SignerSourceKind::Filepath(path) => match read_keypair_file(&path) {
            Err(e) => Err(std::io::Error::new(
                ...
                format!(
                    ...
                    Run \"wickandbergamot-keygen new\" to create a keypair file: {}",
                    path, e
                ),
            )
            .into()),
...
/// ```no_run
/// use clap::{Arg, Command};
/// use wickandbergamot_clap_v3_utils::keypair::keypair_from_path;
///
/// let clap_app = Command::new("my-program")
///     // The argument we'll parse as a signer "path"
///     .arg(Arg::new("keypair")
...
mod tests {
    use {
        ...
        clap::{Arg, Command},
        wickandbergamot_remote_wallet::{locator::Manufacturer, remote_wallet::initialize_wallet_manager},
