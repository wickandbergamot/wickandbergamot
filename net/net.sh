...
here=$(dirname "$0")
WICKANDBERGAMOT_ROOT="$(cd "$here"/..; pwd)"
...
Operate a configured testnet
 ...
 sanity/start-specific options:
   ...
   -o noInstallCheck    - Skip wickandbergamot-install sanity
 ...
 update-specific options:
   --platform linux|osx|windows       - Deploy the tarball using 'wickandbergamot-install deploy ...' for the
...
build() {
  ...
  if [[ $(uname) != Linux || ! " ${supported[*]} " =~ $(lsb_release -sr) ]]; then
    ...
    source "$WICKANDBERGAMOT_ROOT"/ci/rust-version.sh
    MAYBE_DOCKER="ci/docker-run.sh $rust_stable_docker_image"
  fi
  SECONDS=0
  (
    cd "$WICKANDBERGAMOT_ROOT"
  ...
  (
    ...
    (
      ...
      echo "commit: $COMMIT"
    ) > "$WICKANDBERGAMOT_ROOT"/farf/version.yml
  )
  echo "Build took $SECONDS seconds"
}

WICKANDBERGAMOT_HOME="\$HOME/solana"
CARGO_BIN="\$HOME/.cargo/bin"

startCommon() {
  declare ipAddress=$1
  test -d "$WICKANDBERGAMOT_ROOT"
  if $skipSetup; then
    # shellcheck disable=SC2029
    ssh "${sshOptions[@]}" "$ipAddress" "
      set -x;
      mkdir -p $WICKANDBERGAMOT_HOME/config;
      rm -rf ~/config;
      mv $WICKANDBERGAMOT_HOME/config ~;
      rm -rf $WICKANDBERGAMOT_HOME;
      mkdir -p $WICKANDBERGAMOT_HOME $CARGO_BIN;
      mv ~/config $WICKANDBERGAMOT_HOME/
    "
  else
    # shellcheck disable=SC2029
    ssh "${sshOptions[@]}" "$ipAddress" "
...
syncScripts() {
  ...
  rsync -vPrc -e "ssh ${sshOptions[*]}" \
    --exclude 'net/log*' \
    "$WICKANDBERGAMOT_ROOT"/{fetch-perf-libs.sh,fetch-spl.sh,scripts,net,multinode-demo} \
    "$ipAddress":"$WICKANDBERGAMOT_HOME"/ > /dev/null
}
...
deployBootstrapValidator() {
  ...
  tar)
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$WICKANDBERGAMOT_ROOT"/solana-release/bin/* "$ipAddress:$CARGO_BIN/"
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$WICKANDBERGAMOT_ROOT"/solana-release/version.yml "$ipAddress:~/"
    ;;
  local)
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$WICKANDBERGAMOT_ROOT"/farf/bin/* "$ipAddress:$CARGO_BIN/"
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$WICKANDBERGAMOT_ROOT"/farf/version.yml "$ipAddress:~/"
    ;;
  skip)
    ;;
  *)
...
startClients() {
  for ((i=0; i < "$numClients" && i < "$numClientsRequested"; i++)) do
    if [[ $i -lt "$numBenchTpsClients" ]]; then
      startClient "${clientIpList[$i]}" "wickandbergamot-bench-tps" "$i"
    else
...
deployUpdate() {
  ...
  for updatePlatform in $updatePlatforms; do
    echo "--- Deploying wickandbergamot-install update: $updatePlatform"
    (
      set -x

      scripts/wickandbergamot-install-update-manifest-keypair.sh "$updatePlatform"
...
prepareDeploy() {
  ...
  tar)
    if [[ -n $releaseChannel ]]; then
      ...
      rm -f "$WICKANDBERGAMOT_ROOT"/solana-release.tar.bz2
      ...
      (
        ...
        curl -L --retry 5 --retry-delay 2 --retry-connrefused \
          -o "$WICKANDBERGAMOT_ROOT"/solana-release.tar.bz2 "$updateDownloadUrl"
      )
      tarballFilename="$WICKANDBERGAMOT_ROOT"/solana-release.tar.bz2
    fi
    (
      set -x
      rm -rf "$WICKANDBERGAMOT_ROOT"/solana-release
      cd "$WICKANDBERGAMOT_ROOT"; tar jfxv "$tarballFilename"
      cat "$WICKANDBERGAMOT_ROOT"/solana-release/version.yml
    )
    ;;
  local)
    if $doBuild; then
  ...
  *)
  ...
  if [[ -n $deployIfNewer ]]; then
    ...
    if ! diff -q current-version.yml "$WICKANDBERGAMOT_ROOT"/solana-release/version.yml; then
...
deploy() {
  ...
  tar)
    networkVersion="$(
      (
        set -o pipefail
        grep "^commit: " "$WICKANDBERGAMOT_ROOT"/solana-release/version.yml | head -n1 | cut -d\  -f2
      ) || echo "tar-unknown"
    )"
    ;;
  local)
...
netem)
  if [[ -n $netemConfigFile ]]; then
    ...
    if [[ $netemCommand = "add" ]]; then
      for ipAddress in "${validatorIpList[@]}"; do
        "$here"/scp.sh "$netemConfigFile" solana@"$ipAddress":"$WICKANDBERGAMOT_HOME"
      done
    fi
