...
Creates a testnet dev metrics database
  ...
  -e              Assume database already exists and SAFEANA_METRICS_CONFIG is
...
if $useEnv; then
  [[ -n $SAFEANA_METRICS_CONFIG ]] ||
    usage "SAFEANA_METRICS_CONFIG is not defined in the environment"
else
  username=$1
  ...
  SAFEANA_METRICS_CONFIG="host=$host,db=$netBasename,u=scratch_writer,p=topsecret"
fi

echo "export SAFEANA_METRICS_CONFIG=\"$SAFEANA_METRICS_CONFIG\"" >> "$configFile"
