...
declare -r SAFEANA_LOCK_FILE="/home/solana/.solana.lock"
...
colo_whoami() {
  ...
  done < <(colo_instance_run_foreach "[ -n \"\${SAFEANA_USER}\" ] && echo -e \"\${SAFEANA_USER}\\x1fEOL\"")
  echo "${ME}"
}

COLO_SAFEANA_USER=""
colo_get_solana_user() {
  if [ -z "${COLO_SAFEANA_USER}" ]; then
    COLO_SAFEANA_USER=$(colo_whoami)
  fi
  echo "${COLO_SAFEANA_USER}"
}
...
__colo_node_status_script() {
  ...
  exec 2>/dev/null  # Suppress stderr as the next call to exec fails most of
                    # the time due to ${SAFEANA_LOCK_FILE} not existing and is running from a
                    # subshell where normal redirection doesn't work
  exec 9<"${SAFEANA_LOCK_FILE}" && flock -s 9 && . "${SAFEANA_LOCK_FILE}" && exec 9>&-
  echo -e "\${SAFEANA_LOCK_USER}\\x1f\${SAFEANA_LOCK_INSTANCENAME}\\x1f\${PREEMPTIBLE}\\x1fEOL"
