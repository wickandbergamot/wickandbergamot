...
if [[ -f "${WICKANDBERGAMOT_LOCK_FILE}" ]]; then
  ...
  . "${WICKANDBERGAMOT_LOCK_FILE}"
  if [[ "${WICKANDBERGAMOT_LOCK_USER}" = "${WICKANDBERGAMOT_USER}"  || -n "${FORCE_DELETE}" ]]; then
...
EOAK
    ...
    echo "Invalid user: expected \"${WICKANDBERGAMOT_LOCK_USER}\" got \"${WICKANDBERGAMOT_USER}\"" 1>&2
  fi
  exec 9>&-
fi
${RC}
