...
(
  ...
  cp "${RELEASE_BASENAME}"/bin/wickandbergamot-install-init wickandbergamot-install-init-$TARGET
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml wickandbergamot-install-init-"$TARGET"* $MAYBE_TARBALLS; do
