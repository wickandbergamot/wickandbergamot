...
The Wickandbergamot runtime allows programs to call each other via a mechanism called
...
`invoke()` is built into Wickandbergamot's runtime and is responsible for routing the
...
Wickandbergamot, they must each transfer their wager's assets to some intermediary that
will honor their agreement. Currently, there is no way to implement this
intermediary as a program in Wickandbergamot because the intermediary program cannot
