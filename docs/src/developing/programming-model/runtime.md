...
The current [compute
budget](https://github.com/wickandbergamot/wickandbergamot/blob/090e11210aa7222d8295610a6ccac4acda711bb9/program-runtime/src/compute_budget.rs#L26-L87) can be found in the Wickandbergamot Program Runtime.

#### Example Compute Budget

For example, if the compute budget set in the Wickandbergamot runtime is:
...
`SetComputeUnitPrice`
[Compute budget instructions](https://github.com/wickandbergamot/wickandbergamot/blob/db32549c00a1b5370fcaf128981ad3323bbd9570/sdk/src/compute_budget.rs#L22)
respectively.
...
budget instructions](https://github.com/wickandbergamot/wickandbergamot/blob/db32549c00a1b5370fcaf128981ad3323bbd9570/sdk/src/compute_budget.rs#L22)) and the default per-instruction units, which is currently 200k.
...
As Wickandbergamot evolves, new features or patches may be introduced that changes the
...
coordinate, then these changes can result in a break-down of consensus. Wickandbergamot supports a mechanism called runtime features to facilitate the smooth adoption
of changes.

Runtime features are epoch coordinated events where one or more behavior changes
to the cluster will occur. New changes to Wickandbergamot that will change behavior are
wrapped with feature gates and disabled by default. The Wickandbergamot tools are then
used to activate a feature, which marks it pending, once marked pending the
feature will be activated at the next epoch.

To determine which features are activated use the [Wickandbergamot command-line
tools](cli/install-wickandbergamot-cli-tools.md):

```bash
wickandbergamot feature status
```

If you encounter problems, first ensure that the Wickandbergamot tools version you are
using match the version returned by `wickandbergamot cluster-version`. If they do not
match, [install the correct tool suite](cli/install--cli-tools.md).
