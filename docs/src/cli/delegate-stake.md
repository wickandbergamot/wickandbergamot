...
After you have [received WICKANDBERGAMOT](transfer-tokens.md), you might consider putting
it to use by delegating _stake_ to a validator. Stake is what we call tokens
in a _stake account_. wickandbergamot weights validator votes by the amount of stake
delegated to them, which gives those validators more influence in determining
then next valid block of transactions in the blockchain. wickandbergamot then generates
new WICKANDBERGAMOT periodically to reward stakers and validators. You earn more rewards
...
```bash
wickandbergamot-keygen new --no-passphrase -o stake-account.json
```
...
Copy the public key and store it for WICKANDBERGAMOTkeeping. You will need it any time you
want to perform an action on the stake account you create next.

Now, create a stake account:

```bash
wickandbergamot create-stake-account --from <KEYPAIR> stake-account.json <AMOUNT> \
...
View the new stake account with the `wickandbergamot stake-account` command:

```bash
wickandbergamot stake-account <STAKE_ACCOUNT_ADDRESS>
```

The output will look similar to this:

```text
Total Stake: 5000 WICKANDBERGAMOT
...
`wickandbergamot stake-authorize` command. For example, to set a new stake authority,
run:

```bash
wickandbergamot stake-authorize <STAKE_ACCOUNT_ADDRESS> \
...
```bash
wickandbergamot create-stake-account --from <KEYPAIR> <STAKE_ACCOUNT_KEYPAIR> --seed <STRING> <AMOUNT> \
...
and seed string. To see what stake address the command will derive, use `wickandbergamot create-address-with-seed`:

```bash
wickandbergamot create-address-with-seed --from <PUBKEY> <SEED_STRING> STAKE
```

`<PUBKEY>` is the public key of the `<STAKE_ACCOUNT_KEYPAIR>` passed to
`wickandbergamot create-stake-account`.
...
accounts with the `wickandbergamot validators` command:
...
address in `wickandbergamot delegate-stake`:

```bash
wickandbergamot delegate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <VOTE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
After delegating stake, use `wickandbergamot stake-account` to observe the changes
...
Delegated Stake: 4999.99771712 WICKANDBERGAMOT
...
Once delegated, you can undelegate stake with the `wickandbergamot deactivate-stake`
command:

```bash
wickandbergamot deactivate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
Transfer tokens out of a stake account with the `wickandbergamot withdraw-stake` command:

```bash
wickandbergamot withdraw-stake --withdraw-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <RECIPIENT_ADDRESS> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
...
existing stake account to a new one, use the `wickandbergamot split-stake` command:

```bash
wickandbergamot split-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <NEW_STAKE_ACCOUNT_KEYPAIR> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
