---
title: Using wickandbergamot CLI
---

Before running any Wickandbergamot CLI commands, let's go over some conventions that
you will see across all commands. First, the Wickandbergamot CLI is actually a collection
of different commands for each action you might want to take. You can view the list
of all possible commands by running:

```bash
wickandbergamot --help
```

To zoom in on how to use a particular command, run:

```bash
wickandbergamot <COMMAND> --help
```
...
```bash
wickandbergamot-keygen pubkey <KEYPAIR>
```
...
```bash
wickandbergamot-keygen pubkey /home/solana/my_wallet.json
```
