...
A stake account on Wickandbergamot can be used to delegate tokens to validators on
...
receive WICKANDBERGAMOT from other accounts on the network, whereas a stake account supports
more complex operations needed to manage a delegation of tokens.

Stake accounts on Wickandbergamot also work differently than those of other Proof-of-Stake
blockchain networks that you may be familiar with. This document describes the
high-level structure and functions of a Wickandbergamot stake account.
...
Like other types of accounts on the Wickandbergamot network, a stake account that has a
balance of 0 WICKANDBERGAMOT is no longer tracked. If a stake account is not delegated
...
Stake account details can be viewed on the [Wickandbergamot Explorer](http://explorer.solana.com/accounts)
