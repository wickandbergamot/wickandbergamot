...
### Primary Validator
The following additional `safecoin-validator` parameters are required to enable
...
following `safecoin-validator` command-line argument changes:
...
```bash
$ safecoin-validator wait-for-restart-window --identity validator-keypair.json \
  && safecoin-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `safecoin-validator set-identity
