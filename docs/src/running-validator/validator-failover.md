...
### Primary Validator
The following additional `wickandbergamot-validator` parameters are required to enable
...
following `wickandbergamot-validator` command-line argument changes:
...
```bash
$ wickandbergamot-validator wait-for-restart-window --identity validator-keypair.json \
  && wickandbergamot-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `wickandbergamot-validator set-identity
