...
```bash
wickandbergamot catchup ~/validator-keypair.json
```
...
this step, you should see the “validator-stake-keypair.json” in your Wickandbergamot
runtime directory.

```bash
wickandbergamot-keygen new -o ~/validator-stake-keypair.json
```

## Delegate Stake

Now delegate 1 WICKANDBERGAMOT to your validator by first creating your stake account:

```bash
wickandbergamot create-stake-account ~/validator-stake-keypair.json 1
```

and then delegating that stake to your validator:

```bash
wickandbergamot delegate-stake ~/validator-stake-keypair.json ~/vote-account-keypair.json
```

> Don’t delegate your remaining WICKANDBERGAMOT, as your validator will use those tokens to vote.
...
```bash
wickandbergamot delegate-stake ~/validator-stake-keypair.json ~/some-other-vote-account-keypair.json
```
...
- View your vote account:`wickandbergamot vote-account ~/vote-account-keypair.json` This displays the current state of all the votes the validator has submitted to the network.
- View your stake account, the delegation preference and details of your stake:`wickandbergamot stake-account ~/validator-stake-keypair.json`
- `wickandbergamot validators` displays the current active stake of all validators, including yours
- `wickandbergamot stake-history` shows the history of stake warming up and cooling down over recent epochs
...
- Once your stake is warmed up, you will see a stake balance listed for your validator by running `wickandbergamot validators`

## Monitor Your Staked Validator

Confirm your validator becomes a [leader](../terminology.md#leader)

- After your validator is caught up, use the `wickandbergamot balance` command to monitor the earnings as your validator is selected as leader and collects transaction fees
- Wickandbergamot nodes offer a number of useful JSON-RPC methods to return information about the network and your validator's participation. Make a request by using curl \(or another http client of your choosing\), specifying the desired method in JSON-RPC-formatted data. For example:
...
- `getVoteAccounts` This will tell you how much active stake your validator currently has. A % of the validator's stake is activated on an epoch boundary. You can learn more about staking on Wickandbergamot [here](../cluster/stake-delegation-and-rewards.md).
...
```bash
wickandbergamot deactivate-stake ~/validator-stake-keypair.json
```
