...
They are implemented as a Wickandbergamot Program, the mechanics of which can be read
...
```bash
wickandbergamot-keygen new -o nonce-keypair.json
wickandbergamot create-nonce-account nonce-keypair.json 1
```
...
```bash
wickandbergamot nonce nonce-keypair.json
```
...
```bash
wickandbergamot new-nonce nonce-keypair.json
```
...
```bash
wickandbergamot nonce-account nonce-keypair.json
```

- Output

```text
balance: 0.5 WICKANDBERGAMOT
minimum balance required: 0.00136416 WICKANDBERGAMOT
...
```bash
wickandbergamot withdraw-from-nonce-account nonce-keypair.json ~/.config/solana/id.json 0.5
```
...
```bash
wickandbergamot authorize-nonce-account nonce-keypair.json nonce-authority.json
```
...
Here we demonstrate Alice paying Bob 1 WICKANDBERGAMOT using a durable nonce. The procedure
...
```bash
$ wickandbergamot-keygen new -o alice.json
$ wickandbergamot-keygen new -o nonce.json
$ wickandbergamot-keygen new -o bob.json
```

#### - Fund Alice's account

Alice will need some funds to create a nonce account and send to Bob. Airdrop
her some WICKANDBERGAMOT

```bash
$ wickandbergamot airdrop -k alice.json 1
1 WICKANDBERGAMOT
```
...
```bash
$ wickandbergamot create-nonce-account -k alice.json nonce.json 0.1
...
```bash
$ wickandbergamot transfer -k alice.json --blockhash expiredDTaxfagttWjQweib42b6ZHADSx94Tw8gHx11 bob.json 0.01
[2020-01-02T18:48:28.462911000Z ERROR wickandbergamot_cli::cli] Io(Custom { kind: Other, error: "Transaction \"33gQQaoPc9jWePMvDAeyJpcnSPiGUAdtVg8zREWv4GiKjkcGNufgpcbFyRKRrA25NkgjZySEeKue5rawyeH5TzsV\" failed: None" })
...
```bash
$ wickandbergamot nonce-account nonce.json
balance: 0.1 WICKANDBERGAMOT
minimum balance required: 0.00136416 WICKANDBERGAMOT
nonce: F7vmkY3DTaxfagttWjQweib42b6ZHADSx94Tw8gHx3W7
```

```bash
$ wickandbergamot transfer -k alice.json --blockhash F7vmkY3DTaxfagttWjQweib42b6ZHADSx94Tw8gHx3W7 --nonce nonce.json bob.json 0.01
HR1368UKHVZyenmH7yVz5sBAijV6XAPeWbEiXEGVYQorRMcoijeNAbzZqEZiH8cDB8tk65ckqeegFjK8dHwNFgQ
```

#### - Success!

The transaction succeeds! Bob receives 0.01 WICKANDBERGAMOT from Alice and Alice's stored
nonce advances to a new value

```bash
$ wickandbergamot balance -k bob.json
0.01 WICKANDBERGAMOT
```
