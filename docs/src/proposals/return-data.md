...
The existing solution that wickandbergamot uses, writes the return data to the callee account data.
...
strings in the [stable log](https://github.com/wickandbergamot/wickandbergamot/blob/95292841947763bdd47ef116b40fc34d0585bca8/sdk/src/process_instruction.rs#L275-L281).

## Note on returning errors

Solidity on Ethereum allows the contract to return an error in the return data. In this case, all
the account data changes for the account should be reverted. On Wickandbergamot, any non-zero exit code
