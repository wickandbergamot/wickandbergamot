...
Messages transmitted to Wickandbergamot validators must not exceed the IPv6 MTU size to
ensure fast and reliable network transmission of cluster info over UDP.
Wickandbergamot's networking stack uses a conservative MTU size of 1280 bytes which,
after accounting for headers, leaves 1232 bytes for packet data like serialized
transactions.

Developers building applications on Wickandbergamot must design their on-chain program
...
deploying Wickandbergamot programs.
...
affect transaction processing in the Wickandbergamot program runtime beyond the
...
because it adds an extra step for transaction processing. Instead, Wickandbergamot
...
Wickandbergamot can provide a new on-chain program which allows "Big" transactions to be
