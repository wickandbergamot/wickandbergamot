...
Applications send transactions to a Wickandbergamot cluster and query validators to confirm the transactions were processed and to check each transaction's result. When the cluster doesn't behave as anticipated, it could be for a number of reasons:
...
The highest level implementation, ThinClient, targets a Wickandbergamot cluster, which may be a deployed testnet or a local cluster running on a development machine.
