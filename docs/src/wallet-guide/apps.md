...
[Zelcore](https://zelcore.io) is a multi-currency wallet now supporting SAFE and all Safecoin tokens (SPL). Each Zelcore account has 3 separate addresses for each asset.
Store, transact, connect to dapps, and (soon) DEX trade in a single app and hold SAFE tokens alongside BTC, ETH, and 270+ other cryptos.
...
Zelcore also uses a custom Safelet-based wallet adapter solution so users can connect to all their dapps with a single browser tab to keep things tidy.

__**DEVS**__: integrate Zelcore wallet into your products by implementing Safelet solution using "link.zelcore.io" as connection URL.
