...
This page describes how to use a Ledger Nano S or Nano X to interact with wickandbergamot
using the command line tools.

## Before You Begin

- [Set up a Nano with the wickandbergamot App](https://support.ledger.com/hc/en-us/articles/360016265659-wickandbergamot-WICKANDBERGAMOT-?docs=true)
- [Install the wickandbergamot command-line tools](../../cli/install-wickandbergamot-cli-tools.md)

## Use Ledger Nano with wickandbergamot CLI

1. Ensure the Ledger Live application is closed
2. Plug your Nano into your computer's USB port
3. Enter your pin and start the wickandbergamot app on the Nano
...
```bash
wickandbergamot-keygen pubkey usb://ledger
```

This confirms your Ledger device is connected properly and in the correct state
to interact with the wickandbergamot CLI. The command returns your Ledger's unique
...
To view any address, use the `wickandbergamot-keygen pubkey` command, as shown below,
...
```bash
wickandbergamot-keygen pubkey usb://ledger
wickandbergamot-keygen pubkey usb://ledger?key=0
wickandbergamot-keygen pubkey usb://ledger?key=1
wickandbergamot-keygen pubkey usb://ledger?key=2
```
...
Any of the addresses displayed by these commands are valid wickandbergamot wallet
...
`wickandbergamot balance` command:

```bash
wickandbergamot balance SOME_WALLET_ADDRESS
```
...
```bash
wickandbergamot balance 7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri
```
...
Note: Any address with a balance of 0 WICKANDBERGAMOT, such as a newly created one on your
Ledger, will show as "Not Found" in the explorer. Empty accounts and non-existent
accounts are treated the same in wickandbergamot. This will change when your account
address has some WICKANDBERGAMOT in it.

### Send WICKANDBERGAMOT from a Nano
...
unlocked with the PIN, Ledger Live is not running, and the wickandbergamot App is open
on the device, showing "Application is Ready".

The `wickandbergamot transfer` command is used to specify to which address to send tokens,
...
```bash
wickandbergamot transfer RECIPIENT_ADDRESS AMOUNT --keypair KEYPAIR_URL_OF_SENDER
```
...
is entered to send `1` WICKANDBERGAMOT to the recipient address `7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri`.
...
```bash
~$ wickandbergamot-keygen pubkey usb://ledger?key=42
CjeqzArkZt6xwdnZ9NZSf8D1CNJN1rjeFiyd8q7iLWAV

~$ wickandbergamot balance CjeqzArkZt6xwdnZ9NZSf8D1CNJN1rjeFiyd8q7iLWAV
1.000005 WICKANDBERGAMOT

~$ wickandbergamot transfer 7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri 1 --keypair usb://ledger?key=42
...
finalized on the wickandbergamot network. You can view details of this or any other
...
When the URL is not fully qualified, the wickandbergamot CLI will prompt you with
...
URLs using the wickandbergamot CLI `resolve-signer` command. For example, try
connecting a Nano to USB, unlock it with your pin, and running the
following command:

```text
wickandbergamot resolve-signer usb://ledger?key=0/0
```
