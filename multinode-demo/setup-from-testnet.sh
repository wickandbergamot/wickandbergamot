...
rm -rf "$WICKANDBERGAMOT_CONFIG_DIR"/latest-testnet-snapshot
mkdir -p "$WICKANDBERGAMOT_CONFIG_DIR"/latest-testnet-snapshot
(
  cd "$WICKANDBERGAMOT_CONFIG_DIR"/latest-testnet-snapshot || exit 1
  set -x
  wget http://api.testnet.wickandbergamot.org/genesis.tar.bz2
  wget --trust-server-names http://testnet.wickandbergamot.org/snapshot.tar.bz2
)

snapshot=$(ls "$WICKANDBERGAMOT_CONFIG_DIR"/latest-testnet-snapshot/snapshot-[0-9]*-*.tar.zst)
if [[ -z $snapshot ]]; then
...
$WICKANDBERGAMOT_ledger_tool create-snapshot \
  --ledger "$WICKANDBERGAMOT_CONFIG_DIR"/latest-testnet-snapshot \
